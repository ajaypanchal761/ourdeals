...
function CategorySubcategoriesPage() {
  ...
  useEffect(() => {
    const fetchSubcategories = async () => {
      ...
      setParentCategory(decodedCategoryName)

      // Get category ID from mapping
      const categoryId = categoryIdMap[decodedCategoryName]

      // If category ID is available, fetch from API
      if (categoryId) {
        setLoading(true)
        setError(null)
        ...
        try {
          ...
          if (response.status === true && response.data && Array.isArray(response.data)) {
            const transformedSubcategories = response.data.map(item => ({
              id: item.id || item.subcategory_id,
              name: item.name || item.subcategory_name || item.title || item.sub_cat_name || 'Subcategory',
              image: getImageUrl(item.image || item.icon || '')
            }))
        ...
        setError('No category mapping found')
        setSubcategories([])
      }
    }
  ...
  // Fetch wedding banners if category is "Wedding Requisites"
  useEffect(() => {
    const fetchWeddingBanners = async () => {
      if (!categoryName) return

      const decodedCategoryName = decodeURIComponent(categoryName)

      // Only fetch wedding banners for "Wedding Requisites" category
      if (decodedCategoryName === 'Wedding Requisites') {
        try {
          const response = await getAllBanners()
          if (response?.status === true && response?.data && Array.isArray(response.data)) {
            // Find "Shadi Banners" (id: 4)
            ...
            if (weddingBannerData && weddingBannerData.images && Array.isArray(weddingBannerData.images)) {
              const bannerImages = weddingBannerData.images.map(img => getImageUrl(img))
              setWeddingBannerImages(bannerImages)
            }
          }
        } catch (err) {
          console.error('Error fetching wedding banners:', err)
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <div className="bg-[#13335a] px-[clamp(16px,2vw,24px)] md:px-[clamp(12px,1.5vw,16px)] py-[clamp(16px,2vw,20px)] flex justify-between items-center fixed top-0 left-0 right-0 z-[1000] shadow-[0_2px_8px_rgba(0,0,0,0.15)]">
        ...
        <h2 className="text-[clamp(18px,2.5vw,24px)] md:text-[clamp(18px,2.2vw,22px)] font-bold text-white m-0 flex-1 text-center"><TranslatedText>{parentCategory || 'Category'}</TranslatedText></h2>
        <div style={{ width: '24px' }}></div>
      </div>

      {/* Content with top padding so it doesn't hide behind fixed header */}
      <div className="flex-1 p-[clamp(16px,2vw,24px)] pt-[clamp(72px,10vw,96px)] md:p-[clamp(12px,1.5vw,16px)] md:pt-[clamp(80px,10vw,100px)] overflow-y-auto pb-[100px] bg-white">
        {/* Show wedding banners if category is "Wedding Requisites" */}
        {weddingBannerImages.length > 0 && (
          <div className="mb-[clamp(16px,2vw,24px)]">
            <BannerCarousel bannerSet={1} images={weddingBannerImages} />
          </div>
        )}
        {loading ? (
          ...
          <div className="grid grid-cols-4 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-7 gap-[clamp(8px,1vw,12px)] md:gap-[clamp(6px,0.8vw,10px)] mt-[clamp(16px,2vw,24px)] md:mt-[clamp(12px,1.5vw,16px)]">
            {(translatedSubcategories.length > 0 ? translatedSubcategories : subcategories).map((subcat, index) => {
              ...
              const subcatImage = subcat.image || null
              // If we are using the translated array, the name is already translated.
              // We should NOT use TranslatedText here to avoid double-translation or incorrectly treating Hindi as English.
              const isTranslated = translatedSubcategories.length > 0 && shouldTranslate
              ...
              return (
                ...
                >
                  <div className="w-full aspect-square rounded-[clamp(10px,1.2vw,12px)] overflow-hidden bg-gray-100 flex items-center justify-center">
                    ...
                    <div className={`w-full h-full ${subcatImage ? 'hidden' : 'flex'} items-center justify-center bg-[#13335a] text-white font-bold text-[clamp(16px,2vw,20px)] sm:text-[clamp(18px,2.2vw,24px)] md:text-[clamp(18px,2.3vw,25px)] lg:text-[clamp(20px,2.5vw,28px)]`}>
                  ...
                  <span className="text-[clamp(10px,1.2vw,12px)] sm:text-[clamp(11px,1.3vw,13px)] md:text-[clamp(11px,1.3vw,13px)] lg:text-[clamp(12px,1.4vw,14px)] xl:text-[clamp(12px,1.5vw,15px)] font-bold text-gray-800 text-center leading-[1.4] pt-[clamp(6px,0.8vw,10px)] sm:pt-[clamp(7px,0.9vw,11px)] md:pt-[clamp(6px,0.8vw,8px)] lg:pt-[clamp(8px,1vw,12px)] w-full box-border mt-[clamp(4px,0.6vw,8px)] sm:mt-[clamp(5px,0.7vw,9px)] md:mt-[clamp(4px,0.6vw,6px)] lg:mt-[clamp(6px,0.8vw,10px)] group-hover:text-[#13335a]">
                    {isTranslated ? subcatName : <TranslatedText>{subcatName}</TranslatedText>}
                  </span>
                </div>
              )
            })}
